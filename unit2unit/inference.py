...
def load_model(model_path, src_lang, tgt_lang, use_cuda=False):
    ...
    use_cuda = use_cuda and not cfg.common.cpu
...
def cli_main():
    ...
    parser.add_argument(
        "--in-unit-path", type=str, required=True, nargs="*", help="File path of source unit input"
    )
    parser.add_argument(
        "--out-unit-path", type=str, required=True, nargs="*", help="File path of target unit output"
    )
    parser.add_argument(
